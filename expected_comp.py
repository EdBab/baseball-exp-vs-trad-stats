...
def compute_expected_stat(player, stat):
    ...
    for year in range(begin_year, end_year):
        ...
        try:
            exp_stat = year_stats.loc[year_stats["IDfg"] == player["IDfg"]][
                stat
            ].item()
        except Exception:
            continue
        ...
        if stat == "wOBA":
            pas = year_stats.loc[year_stats["IDfg"] == player["IDfg"]][
                "PA"
            ].item()
            results_dict[year] = [pas, exp_stat]
        else:
            abs = year_stats.loc[year_stats["IDfg"] == player["IDfg"]][
                "AB"
            ].item()
...
def build_comparison(player):
    ...
    prev_SLG = data_begin.loc[data_begin["IDfg"] == player["IDfg"]][
        "SLG"
    ].item()
    new_SLG = data_end.loc[data_end["IDfg"] == player["IDfg"]]["SLG"].item()
    prev_xSLG = compute_expected_stat(player, "xSLG")

    prev_wOBA = data_begin.loc[data_begin["IDfg"] == player["IDfg"]][
        "wOBA"
    ].item()
...
fig_ba.suptitle(
    "Batting Average v. Expected Batting Average as indicator of future"
    " production"
)
fig_slg, ax_slg = plt.subplots(1, 2, figsize=(10, 5))
fig_slg.suptitle(
    "Slugging % v. Expected Slugging % as indicator of future production"
)
